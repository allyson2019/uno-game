let Game = { 
    deck: null, //placeholder for nothing
    players: {},
    turnDirection: 1,
    playersTurn: null,
    topCard: null
...
function startNewGame(){
    ...
    //add up to fout players to the Game object
                         //   0        1     2          3
    const playerNames = ["Harry", "Gordon Ramsey", "Shame Lizard", "Niall"]
    ...
    //let colot = getCardColor(discard)
...
function changePlayersTurn(){
    ...
    //const ALPHABET = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J' ]
    //first get current players turn from game object
    const currentPlayerId = Game.playersTurn
    //then get the turnDirection from the game object
    const currentDirection = Game.turnDirection
    //move the current players turn one position in
    //first get the index of the playerTurn in the alphabet array
    const idx = ALPHABET.indexOf(currentPlayerId)
        //then change that index by the directional number
    let newIdx = idx + currentDirection
        //if.. index is less than 0 set it to the index of last player
    //const keys = Object.kets(Game.players)
    //const num Players = keys.length
    if (newIdx < 0) {
...
let player = Game.players[playerId]
    ...
    if (cardIsPlayable)
    if (val === 'skip')
    //skip from one player to the nect
    if (val === 'reverse')
    //reverse the order of player turn
    if (val === 'draw two')
    //next player takes two cards from the deck
    //it should draw one card
    //off the top of the deck
    //and put them in the players hand
function playerDrawCard(){}
function skipTurn(){}
if Game.players.card =
//take in a player ID
